...
interface CollectionPageProps {
  pageType: 'new-arrivals' | 'bedding' | 'curtains' | 'throws' | 'blankets';
}

const PAGE_CONFIG: Record<
  CollectionPageProps['pageType'],
  {
    title: string;
    subtitle: string;
    description: string;
...
export const CollectionPage: React.FC<CollectionPageProps> = ({ pageType }) => {
  ...
  return (
    <main className="flex-grow bg-[#faf9f7] animate-fadeIn pb-24">
      ...
      <section className="relative w-full h-[50vh] min-h-[400px] flex items-end justify-start overflow-hidden bg-[#efeeec]">
        <img
        ...
        <div className="relative z-10 w-full max-w-[1440px] mx-auto px-5 md:px-16 pb-12 text-white">
          {/* Breadcrumbs */}
          ...
          <h1
            className="text-[36px] sm:text-[52px] leading-[44px] sm:leading-[60px] tracking-[-0.02em] text-white max-w-2xl font-normal"
            style={{ fontFamily: "'Libre Caslon Text', Georgia, serif" }}
          >
            {config.title}
          </h1>
          <p className="text-body-md text-white/80 max-w-xl mt-3 font-light">
            {config.description}
          </p>
        </div>
      </section>
      ...
      <section className="w-full max-w-[1440px] mx-auto px-5 md:px-16 pt-10 pb-8">
        <div className="flex flex-col md:flex-row justify-between items-start md:items-center py-5 border-y border-[#c4c7c7] gap-4">
          ...
          <div className="flex flex-wrap items-center gap-2.5">
            <span className="text-label-caps text-[#000000] uppercase tracking-wider mr-2">Filter:</span>
            {config.filterCategories.map((sub) => (
              <button
                key={sub.id}
                onClick={() => setSelectedSubCategory(sub.id)}
                className={`px-4 py-2 border text-label-caps uppercase transition-all duration-200 ${
                  selectedSubCategory === sub.id
                    ? 'border-[#000000] bg-[#000000] text-white shadow-sm'
                    : 'border-[#c4c7c7] text-[#444748] hover:border-[#000000] hover:text-[#000000] bg-transparent'
                }`}
              >
                {sub.label}
              </button>
          ...
          <div className="flex items-center gap-5 w-full md:w-auto justify-between md:justify-end">
            <span className="text-body-sm text-[#444748] tracking-wide">
              {filteredProducts.length} Pieces
            </span>

            <div className="flex items-center gap-2">
              <span className="text-label-caps text-[#444748]">Sort:</span>
              <select
                value={selectedSort}
      ...
      {/* Product Grid */}
      <section className="w-full max-w-[1440px] mx-auto px-5 md:px-16">
        {filteredProducts.length === 0 ? (
          <div className="text-center py-24 border border-dashed border-[#c4c7c7] p-12">
            ...
            <button
              ...
              className="px-8 py-3 bg-[#000000] text-white text-label-caps hover:bg-[#2f3130] transition-colors"
            >
              Reset Filters
            </button>
          </div>
